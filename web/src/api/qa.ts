...
export async function startQaThread(
  ...
  const res = await http.post<StartQaThreadResponse>(
    `/sessions/${sessionId}/qa-threads`,
    body,
  );
  return res.data;
}
...
export async function appendQaMessage(
  ...
  const res = await http.post<AppendQaMessageResponse>(
    `/qa-threads/${threadId}/messages`,
    body,
  );
  return res.data;
}

export async function endQaThread(
  threadId: number,
): Promise<EndQaThreadResponse> {
  const res = await http.post<EndQaThreadResponse>(
    `/qa-threads/${threadId}/end`,
  );
  return res.data;
}

export async function listActiveQaThreads(
  sessionId: number,
...
export async function listAllQaThreads(
  ...
  const res = await http.get<ListAllQaThreadsResponse>(
    `/sessions/${sessionId}/qa-threads`,
  );
  return res.data;
}
